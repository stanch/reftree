...
object OpticInstances {
  ...
  object Marker extends ProductTypeClassCompanion[Marker] {
    implicit val `Int Marker`: Marker[Int] = new Marker[Int](x => x + 1)
    implicit val `Long Marker`: Marker[Long] = new Marker[Long](x => x + 1L)
    implicit val `Double Marker`: Marker[Double] = new Marker[Double](x => x + 1.0)
    implicit val `Char Marker`: Marker[Char] = new Marker[Char]({ case '?' => '!'; case _ => '?' })
    implicit val `String Marker`: Marker[String] = new Marker[String](x => x + " ")
